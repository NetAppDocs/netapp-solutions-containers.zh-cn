...
此解决方案利用部署在 VMware vSphere 中的虚拟设备。F5 Big-IP 虚拟设备的网络可以根据您的网络环境配置为双臂或三臂配置。本文档中的部署基于双臂配置。有关配置虚拟设备以用于 Anthos 的其他详细信息，请参阅 link:https://cloud.google.com/solutions/partners/installing-f5-big-ip-adc-for-gke-on-prem["适用于 GKE On-Prem 的 F5 BIG-IP ADC 安装指南"^]。
...
. 从 link:https://downloads.f5.com/esd/serveDownload.jsp?path=/big-ip/big-ip_v15.x/15.0.1/english/virtual-edition/&sw=BIG-IP&pro=big-ip_v15.x&ver=15.0.1&container=Virtual-Edition&file=BIGIP-15.0.1-0.0.11.ALL-vmware.ova["F5 下载门户"^] 下载虚拟应用程序 Open Virtual Appliance (OVA) 文件。
+

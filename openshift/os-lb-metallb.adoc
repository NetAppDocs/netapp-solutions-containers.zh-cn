...
* *第 2 层模式。*在此模式下，OpenShift 集群中的一个节点获取服务的所有权，并响应该 IP 的 ARP 请求，使其在 OpenShift 集群外部可访问。由于只有该节点通告 IP，因此存在带宽瓶颈和故障转移速度慢的限制。有关详细信息，请参见 link:https://metallb.universe.tf/concepts/layer2/["MetalLB Layer 2 模式文档"^]。
* *BGP 模式。*在此模式下，OpenShift 集群中的所有节点与路由器建立 BGP 对等会话，并通告路由以将流量转发到服务 IP。其先决条件是将 MetalLB 与该网络中的路由器集成。由于 BGP 中的哈希机制，当服务的 IP 到节点映射发生变化时，存在一定的局限性。有关详细信息，请参阅 link:https://metallb.universe.tf/concepts/bgp/["MetalLB BGP 模式文档"^]。
...
NOTE: 如果要在 BGP 模式下配置 MetalLB，请跳过上述步骤 6 并按照 link:https://metallb.universe.tf/concepts/bgp/["MetalLB BGP 模式文档"^] 中的步骤操作。

...
有关 NetApp ONTAP 存储系统的详细信息，请访问 link:https://www.netapp.com/data-management/ontap-data-management-software/["NetApp ONTAP网站"^]。
...
有关 ONTAP 的详细信息，请参见 link:https://docs.netapp.com/ontap-9/index.jsp["ONTAP 9 文档中心"^]。
...
Amazon FSx ONTAP 在 AWS Cloud 中提供完全托管的共享存储，具备 ONTAP 广受欢迎的数据访问和管理功能。有关 Amazon FSx for NetApp ONTAP 的更多信息，请参见 link:https://docs.aws.amazon.com/fsx/latest/ONTAPGuide/what-is-fsx-ontap.html["Amazon FSx for NetApp ONTAP 文档"^]。
...
Azure NetApp Files 是一种 Azure 原生、第一方、企业级、高性能文件存储服务。它提供卷即服务，您可以为其创建 NetApp 帐户、容量池和卷。您还可以选择服务和性能级别以及管理数据保护。您可以使用熟悉且在本地依赖的相同协议和工具，创建和管理高性能、高可用性和可扩展的文件共享。有关 Azure NetApp Files 的详细信息，请参见 link:https://learn.microsoft.com/en-us/azure/azure-netapp-files/["Azure NetApp Files 文档"^]。
...
Google Cloud NetApp Volumes 是一种完全托管的基于云的数据存储服务，可提供高级数据管理功能和高度可扩展的性能。它允许您将基于文件的应用程序移动到 Google Cloud。它内置支持网络文件系统（NFSv3 和 NFSv4.1）和服务器消息块（SMB）协议，因此您无需重新构建应用程序，并且可以继续为应用程序获取永久存储。有关 Google Cloud NetApp Volumes 的详细信息，请参见 link:https://cloud.google.com/netapp/volumes/docs/discover/overview["Google Cloud NetApp Volumes 文档"^]。

...
+
在运行 Helm 安装之前，创建并标记 `trident`命名空间。 `enforce=privileged`标签是必需的，因为 Trident 运行特权工作负载。没有它，Kubernetes Pod 安全准入控制器将阻止 Trident Pod 启动。
...
+
在手动安装 Trident 之前，请确保已使用所需的 Pod 安全标签标记 `trident`命名空间。
...
NOTE: 如果 Trident Pod 启动失败并出现 `PodSecurity`准入错误，则 Pod 安全标签可能未在安装前应用于 `trident`命名空间。使用 `--overwrite`重新应用它们，然后验证 Pod 是否成功启动。
